...
export default function Course() {
      ...
      if (user && token && courseId) {
        try {
          ...
          if (user.role !== 'admin' && user.role !== 'instructor') {
            const enrolledIds = await apiCall<number[]>('/api/my-enrollments', token);
          ...
          if (enrolled) {
            ...
            const data = await apiCall<ProgressData>(`/api/courses/${courseId}/progress`, token);
  ...
  const fetchCourseDetails = useCallback(async () => {
    if (!courseId) return;
    try {
      const courses = await publicApiCall<Course[]>('/api/courses');
      const found = courses.find((c) => c.id === parseInt(courseId as string));
  ...
  const fetchLessons = useCallback(async () => {
    ...
    try {
      const fetcher = token
        ? (url: string) => apiCall<Lesson[]>(url, token)
        : (url: string) => publicApiCall<Lesson[]>(url);
      const lessonsData = await fetcher(`/api/courses/${courseId}/lessons`);

      const lessonsWithQuiz = await Promise.all(
        lessonsData.map(async (lesson) => {
          try {
            if (token) {
              const quizData = await apiCall<QuizQuestion[]>(
  ...
  const confirmFreeEnrollment = async () => {
    ...
    setIsEnrolling(true);
    try {
      await apiCall<EnrollResponse>('/api/enroll', token, 'POST', { course_id: course.id });
      ...
      const msg = err instanceof Error ? err.message : 'حدث خطأ أثناء الاشتراك.';
      toast.error(msg);
    } finally {
  ...
  const proceedToPayment = async (method: 'card' | 'kiosk') => {
    ...
    setIsEnrolling(true);
    try {
      const response = await apiCall<PaymentInitResponse>('/api/paymob/init', token, 'POST', {
        course_id: course.id,
        method,
      });
      ...
      const msg = err instanceof Error ? err.message : 'حدث خطأ أثناء الاتصال بخدمة الدفع.';
      toast.error(msg);
    } finally {
      setIsEnrolling(false);
    }
  ...
  const isLessonLocked = (
    ...
    if (lesson.is_admin_locked === 1)
      return { locked: true, message: 'هذه المحاضرة مغلقة حالياً من الإدارة.' };
    if (index > 0) {
      ...
      if (!completedLessons.has(prevLesson.id)) {
        return {
          locked: true,
          message:
            'عذراً، يجب إتمام المحاضرة السابقة أولاً لتتمكن من فتح هذه المحاضرة.',
        };
      }
    }
  ...
  const openVideo = (lesson: Lesson, videoUrl: string, vIdx: number, vTotal: number) => {
    ...
    setTimeout(() => {
      document
        .getElementById('video-player-section')
  ...
  const handleVideoCelebration = () => {
    ...
    if (vIdx < vTotal - 1) {
      ...
      if (!alreadyCompleted) {
        if (lesson.hasQuiz) {
          toast.success(
            'ممتاز! لقد أكملت جميع الفيديوهات، يرجى فتح الامتحان لإتمام المحاضرة.'
          );
        } else {
  ...
  const openExam = (lesson: Lesson) => {
    ...
    const videoUrls = lesson.video_url
      ? lesson.video_url.split(/[,|\s]+/).filter((url) => url.trim() !== '')
      : [];
    ...
    if (!allVideosWatched) {
      toast.error(
        'تنبيه! لا يمكنك الدخول للامتحان قبل الانتهاء من مشاهدة جميع أجزاء فيديوهات الشرح للمحاضرة.'
      );
      return;
    }
  ...
  const submitExam = async () => {
    ...
    if (token && activeExamLesson) {
      try {
        const response = await apiCall<QuizSubmissionResponse>(
          '/api/progress/quiz',
          token,
          'POST',
          {
        ...
        if (response.status === 'queued') {
          toast.success(
            response.message ||
              'استلمنا إجاباتك ⏱️. نظراً للضغط الحالي، جاري تصحيح ورقتك وسجلناها في النظام. النتيجة هتظهر في ملفك الشخصي خلال دقايق.',
            { duration: 8000 }
          );
          closeExam();
          return;
  ...
  const courseSettings: CourseSettings = (() => {
    try {
      return course?.metadata ? JSON.parse(course.metadata as string) : {};
    } catch {
      return {};
    }
  })();
  ...
  return (
    <div
      className="min-h-screen bg-page-bg flex flex-col relative"
      id="top-section"
    ...
    >
      ...
      <div className="mx-[5%] my-5 relative">
        <div className="bg-white rounded-2xl overflow-hidden shadow-[0_10px_30px_rgba(0,0,0,0.03)] flex flex-col relative border border-border">
          <img
            src={
              course?.image_url ||
              'https://via.placeholder.com/1200x400/015669/ffffff?text=جاري+التحميل...'
            }
            className="w-full h-[250px] object-cover bg-slate-200"
            alt="غلاف الكورس"
          />
          <div className="p-6 text-center">
            <h2 className="text-[26px] text-primary mb-2.5 font-bold">
            ...
            <div className="flex flex-wrap justify-center items-center gap-4">
              {isUserEnrolled ? (
                ...
                >
                  ...
                  {isEnrolling
                    ? 'جاري التجهيز...'
                    : `اشترك الآن ${
                        course?.is_free === 1 ? '(مجاناً)' : `(${course?.price || 0} ج.م)`
                      }`}
                </button>
              )}
              ...
              {course?.instructor_contact &&
                (isUserEnrolled ? (
                  ...
                  <button
                    onClick={() =>
                      toast.info('يجب الاشتراك في الكورس أولاً لتتمكن من التواصل مع المحاضر.')
                    }
      ...
      {activeLessonId !== null && (
        <div
          ...
          className="mx-[5%] mb-10 flex justify-center animate-fade-in scroll-mt-6"
        >
          <div
            ref={videoContainerRef}
            className={`group bg-black rounded-2xl overflow-hidden relative shadow-[0_20px_50px_rgba(0,0,0,0.3)] flex flex-col w-full max-w-[854px] border border-slate-700 ${
              isFullscreen ? '!max-w-none !w-full !h-full !rounded-none !border-none' : ''
            }`}
          >
            {!isFullscreen && (
              <button
            ...
            <div
              className={`${
                isFullscreen
                  ? `absolute bottom-0 left-0 w-full bg-gradient-to-t from-black/95 via-black/60 to-transparent pb-6 pt-16 px-8 transition-opacity duration-300 ${
                      isVideoPlaying ? 'opacity-0 group-hover:opacity-100' : 'opacity-100'
                    }`
                  : 'bg-[#0f172a] p-4 px-6 border-t border-slate-700'
              } flex flex-col gap-4 flex-shrink-0 z-30`}
            >
              <div
                className="w-full h-2.5 bg-white/20 rounded-md cursor-pointer relative overflow-hidden transition-all hover:h-3.5"
                onClick={seekVideo}
              >
                <div
              ...
              <div className="flex justify-between items-center">
                <div className="flex items-center gap-6">
                  <button
                    ...
                    className="bg-transparent text-white border-none text-2xl cursor-pointer transition-all hover:text-white hover:scale-110 flex items-center justify-center"
                  ...
                  <button
                    onClick={togglePlayPause}
                    className="bg-transparent text-white border-none text-[32px] cursor-pointer transition-all hover:scale-110 flex items-center justify-center"
                    title="تشغيل / إيقاف"
                  >
                    <i
                      className={`fas ${isVideoPlaying ? 'fa-circle-pause' : 'fa-circle-play'}`}
                    ></i>
                  </button>
                  <button
                    onClick={() => skipVideo(10)}
                ...
                <div className="flex items-center gap-5">
                  <button
                    ...
                    className="bg-transparent text-white border border-slate-500 px-3 py-1.5 rounded-lg text-sm font-bold cursor-pointer transition-all hover:bg-white hover:text-slate-900 hover:border-white"
                    title="سرعة التشغيل"
                  >
                    {playbackRate}x
                  </button>
                  <div
                    className="text-slate-300 font-bold text-[14px] font-mono tracking-wide"
                    dir="ltr"
                  >
                  ...
                  <button
                    ...
                    className="bg-transparent text-white border-none text-xl cursor-pointer transition-all hover:text-white hover:scale-110 flex items-center justify-center ml-2"
                    title="ملء الشاشة"
                  >
      ...
      <div className="text-center my-6 mb-4">
        <h3 className="text-[30px] text-primary relative inline-block font-bold after:content-[''] after:absolute after:-bottom-2.5 after:left-1/2 after:-translate-x-1/2 after:w-[60%] after:h-1 after:bg-border after:rounded-sm">
          محتوى الكورس
        </h3>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-10">
            ...
            <p className="text-lg text-text-muted">
              المحتوى قيد التجهيز، سيتم إضافة المحاضرات قريباً.
            </p>
          </div>
        ) : (
          lessons.map((lesson, index) => {
            ...
            const rawVideoUrls = lesson.video_url
              ? lesson.video_url.split(/[,|\s]+/).filter((url) => url.trim() !== '')
              : [];
              ...
              <div
                ...
                className={`bg-white rounded-2xl shadow-[0_4px_15px_rgba(0,0,0,0.05)] overflow-hidden transition-all border-2 ${
                  isCompleted
                    ...
                    : 'border-transparent hover:-translate-y-0.5 hover:shadow-[0_8px_25px_rgba(0,0,0,0.08)]'
                }`}
              >
                <div
                ...
                >
                  <div className="flex flex-col gap-1.5 flex-1 pl-4">
                    <div className="text-2xl font-bold text-text-main flex items-center gap-2.5">
                      <i
                        className={`${
                          locked && isUserEnrolled ? 'fas fa-lock' : 'fas fa-border-all'
                        } ${
                          isCompleted
                            ? 'text-success'
                            : locked && isUserEnrolled
                            ? 'text-text-muted'
                            : 'text-red-500'
                        } text-[35px] ml-4 opacity-80`}
                      ></i>
                      المحاضرة {index + 1}{' '}
                      {locked && isUserEnrolled && (
                        <span className="text-danger text-sm">
                          ({lesson.is_admin_locked === 1 ? 'مغلقة من الإدارة' : 'مقفولة تتابعياً'})
                        </span>
                      )}
                    </div>
                  ...
                  <div
                    className={`bg-page-bg w-10 h-10 rounded-full flex justify-center items-center text-lg text-text-main transition-all ${
                  ...
                  <div className="p-5 flex flex-col gap-4 bg-[#fdfdfd] border-t border-border">
                    {displayVideoUrls.map((vUrl, vIdx) => {
                      const isVideoCompleted =
                        isUserEnrolled &&
                        (completedVideos.has(`${lesson.id}_${vIdx}`) || isCompleted);
                      const isActiveVideo =
                        isUserEnrolled &&
                        ytDataRef.current.lesson?.id === lesson.id &&
                        ytDataRef.current.vIdx === vIdx &&
                        activeLessonId !== null;
                        ...
                        >
                            ...
                            <i
                              className={`${
                                !isUserEnrolled
                                  ...
                                  ? 'fas fa-circle-check'
                                  : isActiveVideo
                                  ? 'fas fa-circle-play fa-fade'
                                  : 'fas fa-video'
                              } text-2xl`}
                            ></i>
                            <span>
                              جزء الشرح والتدريبات
                              {displayVideoUrls.length > 1 ? ` (الجزء ${vIdx + 1})` : ''}
                            </span>
                          </div>
                          <span className="text-sm text-text-main bg-white py-1.5 px-3 rounded-lg border border-border flex items-center gap-1.5">
                            {!isUserEnrolled
                              ...
                              ? 'يتم العرض الآن'
                              : 'مشاهدة الفيديو'}
                            {isUserEnrolled && !isVideoCompleted && (
      ...
      {/* Enroll confirmation modal (free courses) */}
      {showEnrollConfirmModal && course && (
        <div className="fixed inset-0 bg-slate-900/60 flex justify-center items-center z-[9999] backdrop-blur-sm px-4">
          <div className="bg-white p-8 rounded-[24px] w-full max-w-[400px] text-center shadow-[0_20px_60px_rgba(0,0,0,0.2)] animate-fade-in border border-border relative overflow-hidden">
            ...
            <h2 className="text-[22px] text-slate-800 font-bold mb-3">
      ...
      {showPaymentMethodModal && course && (
        <div className="fixed inset-0 bg-slate-900/60 flex justify-center items-center z-[9999] backdrop-blur-sm px-4">
          <div className="bg-white p-8 rounded-[24px] w-full max-w-[450px] text-center shadow-[0_20px_60px_rgba(0,0,0,0.2)] animate-fade-in border border-border relative overflow-hidden">
            ...
            <p className="text-text-muted mb-8 text-[15px] leading-relaxed px-2">
              للاشتراك في الكورس بقيمة{' '}
              <strong className="text-primary">{course.price} ج.م</strong>، يرجى اختيار الطريقة
              الأنسب لك:
            </p>
      ...
      {showPaymentModal && paymentReference && (
        <div className="fixed inset-0 bg-slate-900/60 flex justify-center items-center z-[9999] backdrop-blur-sm px-4">
          <div className="bg-white p-8 rounded-[24px] w-full max-w-[480px] text-center shadow-[0_20px_60px_rgba(0,0,0,0.2)] animate-fade-in border border-border relative overflow-hidden">
            ...
            <h2 className="text-[24px] text-slate-800 font-bold mb-2">
              كود الدفع (بيموب / فوري)
            </h2>
            <p className="text-text-muted mb-6 text-[15px]">
              يرجى التوجه لأي منفذ فوري أو أمان واطلب الدفع لخدمة (بيموب / Paymob) باستخدام هذا
              الكود المرجعي.
            </p>
            ...
            <div className="flex flex-col gap-3 mb-8 text-right">
              <div className="bg-blue-50 text-blue-700 p-4 rounded-xl text-sm font-bold flex items-start gap-3 border border-blue-100">
                ...
                <p className="m-0 leading-relaxed">
                  الكود صالح لمدة 24 ساعة فقط. يمكنك الدفع عبر أي ماكينة فوري، أمان، مصاري، أو من
                  خلال المحافظ الإلكترونية (كود خدمة بيموب).
                </p>
              </div>
              <div className="bg-amber-50 text-amber-700 p-4 rounded-xl text-[13px] font-bold flex items-start gap-3 border border-amber-100">
                ...
                <p className="m-0 leading-relaxed">
                  ملاحظة هامة: بعد إتمام الدفع، قد تستغرق العملية من 5 إلى 30 دقيقة لتسميع البيانات
                  في سيرفراتنا. بمجرد التأكيد، سيتحول الكورس إلى &quot;تم الاشتراك بنجاح&quot; ويمكنك
                  متابعة التعلم فوراً عند تحديث الصفحة.
                </p>
              </div>
            </div>
      ...
      {showExamModal && activeExamLesson && isUserEnrolled && (
        <div className="fixed top-0 left-0 w-full h-full bg-slate-50/95 z-[2000] flex flex-col overflow-hidden select-none">
          <div
            ...
            className="absolute top-0 left-0 w-full h-full bg-black text-white z-[3000] hidden flex-col justify-center items-center text-center p-5"
            onClick={(e) => {
          ...
          >
            ...
            <p className="text-base font-normal mt-2.5 leading-relaxed">
          ...
          <div className="bg-white py-5 px-[5%] flex justify-between items-center border-b border-border shadow-[0_4px_15px_rgba(0,0,0,0.02)]">
            <h2 className="text-primary text-[22px] font-bold">
              <i className="fas fa-pen-to-square ml-2"></i> اختبار: {activeExamLesson.title}
            </h2>
            <button
              onClick={closeExam}
              disabled={isGrading}
              className="bg-red-50 text-red-500 border-none py-2.5 px-5 rounded-xl font-bold cursor-pointer flex items-center gap-2 hover:bg-red-600 hover:text-white transition-all disabled:opacity-50"
            >
          ...
          <div className="flex-1 overflow-y-auto py-8 px-[5%] flex flex-col items-center">
            {isGrading ? (
              ...
              <>
                <div
                  className={`text-white py-2.5 px-6 rounded-[30px] font-bold text-xl mb-5 flex items-center gap-2.5 shadow-[0_5px_15px_rgba(239,68,68,0.3)] ${
                    timeRemaining < 30
                ...
                <h3 className="text-text-muted mb-6 text-lg">
                  السؤال{' '}
                  <span className="text-primary font-bold text-[22px]">{currentQIndex + 1}</span> من{' '}
                ...
                <div className="grid grid-cols-1 gap-4 w-full max-w-[600px]">
                  {(() => {
                    ...
                    const isTrueFalse =
                      (currentQ as QuizQuestion & { type?: string })?.type === 'true_false' ||
                      ...
                      <button
                        ...
                        className={`bg-white border-2 border-border py-4 px-5 rounded-2xl text-lg font-bold cursor-pointer text-right flex items-center gap-4 transition-all text-text-main shadow-[0_4px_10px_rgba(0,0,0,0.02)] hover:border-primary ${
                          userAnswers[currentQIndex] === option
                            ? 'border-primary bg-primary/5 shadow-[0_8px_20px_rgba(1,86,105,0.15)] -translate-y-0.5'
                            : ''
                        }`}
                      >
                        <span
                          className={`w-10 h-10 rounded-full flex justify-center items-center text-primary text-xl flex-shrink-0 ${
                            userAnswers[currentQIndex] === option
              ...
              <div className="flex flex-col items-center justify-center h-full text-center">
                <div
                  className={`w-[150px] h-[150px] rounded-full flex items-center justify-center text-[50px] font-bold text-white mb-5 shadow-[0_10px_30px_rgba(0,0,0,0.1)] ${
                    examScore >= 50 ? 'bg-success' : 'bg-red-500'
                  }`}
                >
                  {examScore}%
                </div>
                <div
                  className={`text-[32px] font-bold mb-2.5 ${
                    examScore >= 50 ? 'text-success' : 'text-red-500'
                  }`}
                >
                  {examScore >= 50
                ...
                <div className="text-xl text-text-muted mb-10">
                  أجبت بشكل صحيح على {Math.round((examScore / 100) * quizQuestions.length)} من أصل{' '}
