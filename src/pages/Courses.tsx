  ...
  const getCourseAction = (course: Course) => {
    ...
    if (isEnrolled) {
      return {
        ...
        action: () => router.push(`/course?id=${course.id}`),
      };
    } else if (isFree) {
      return {
          ...
          <div className="flex items-center gap-3 md:gap-4">
            {isAuthenticated && user ? (
              <>
                ...
                <Link
                  href="/profile"
                  className="bg-primary/10 text-primary no-underline py-2.5 px-4 rounded-xl font-bold transition-all hover:bg-primary hover:text-white hidden md:flex items-center gap-2"
                >
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {courses.map((course) => {
                ...
                let courseSettings: Record<string, string> = {};
                try {
                  if ((course as Record<string, unknown>).metadata) {
                    courseSettings = JSON.parse((course as Record<string, unknown>).metadata as string);
                  }
                } catch (e) {}
